...
public class DeactiveUserDialog  extends JDialog{
	private final JPanel contentPane = new JPanel();
	private JLabel lblMessage;
	private JComboBox<String> employeeIdBox;
	
	public DeactiveUserDialog(WorkFrame parent) {
		super(parent, "Deactive User", true);
		setAlwaysOnTop(true);
		...
		JButton btnDeactiveUser = new JButton("Deactive");
		btnDeactiveUser.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				UserB userB = new UserB();
				String employeeId = employeeIdBox.getSelectedItem().toString();
				int id = Integer.parseInt(employeeId.trim());
				int n = JOptionPane.showConfirmDialog(DeactiveUserDialog.this,
						"Are you sure you want to deactive user '" + employeeId + "' ?", "Confirm User Deactive",
						JOptionPane.YES_NO_CANCEL_OPTION);
				if (n == JOptionPane.YES_OPTION) {
					userB.activeUser(id, false);
					DeactiveUserDialog.this.dispose();
				}
			}
		});

		lblMessage = new JLabel("Please select id of the user that you want to deactive!");
		contentPane.add(lblMessage, "cell 1 1");
		contentPane.add(btnDeactiveUser, "flowx,cell 1 2");
