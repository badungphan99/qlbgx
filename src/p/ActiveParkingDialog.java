...
public class ActiveParkingDialog extends JDialog {
		...
		public ActiveParkingDialog(WorkFrame parent) {
			super(parent, "Active Parking", true);
			setAlwaysOnTop(true);
			...
			String[] parkingIds = parkingB.getAllParkingIdNotActive();
			parkingIdBox = new JComboBox<String>(parkingIds);
			contentPane.add(parkingIdBox, "cell 1 0,growx");

			JButton btnActiveParking = new JButton("Active");
			btnActiveParking.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					int n = JOptionPane.showConfirmDialog(ActiveParkingDialog.this,
							"Are you sure you want to active parking '" + parkingId + "' ?", "Confirm Parking Active",
							JOptionPane.YES_NO_CANCEL_OPTION);
					if (n == JOptionPane.YES_OPTION) {
						parkingB.activeParking(id);
						ActiveParkingDialog.this.dispose();
					}
				}
			});

			lblMessage = new JLabel("Please select id of the parking that you want to active!");
